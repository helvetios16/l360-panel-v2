...
export const useProjectStore = defineStore("project", () => {
	...
	async function fetchProjects() {
		...
		try {
			...
			const rawProjects = (data.projects as Project[]) || (Array.isArray(data) ? (data as Project[]) : []);
	...
	async function updateUnitStatus(projectId: string, unitId: string, newStatus: Unit["status"]) {
		...
		const project = (currentProject.value?.id === projectId ? currentProject.value : null) ||
