...
<template>
	<div class="max-w-7xl mx-auto px-4 pb-20 mt-10">
		<div v-if="projectStore.isLoading" class="flex items-center justify-center py-20">
			<div class="animate-spin w-10 h-10 border-4 border-brand border-t-transparent rounded-full"></div>
		</div>

		<div v-else-if="projectStore.error" class="bg-red-400/10 border border-red-400/20 p-6 rounded-2xl text-red-400 text-center">
			{{ projectStore.error }}
		</div>
				...
				<div class="glass-card sticky top-8">
					...
					<div v-if="selectedUnit" class="space-y-8">
						<div class="p-6 bg-white/[0.02] rounded-[1.5rem] border border-white/5 space-y-1">
							...
							<p class="text-3xl font-bold text-white">MZ {{ selectedUnit.block }} - LT {{ selectedUnit.lot }}</p>
						</div>

						<div class="grid grid-cols-2 gap-4">
							<div class="p-6 bg-white/[0.02] rounded-[1.5rem] border border-white/5 space-y-1">
								<p class="text-[10px] text-slate-500 uppercase tracking-[0.2em] font-bold">Área</p>
					...
					<div v-else class="flex flex-col items-center justify-center py-24 text-center space-y-8">
						<div
