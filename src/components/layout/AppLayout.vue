...
<template>
	<div class="flex h-screen overflow-hidden bg-[#060809]">
		...
		>
			...
			<div class="px-6 h-16 flex items-center border-b border-white/[0.06]">
				<h1 class="text-xl font-bold tracking-tight">
					<span class="bg-linear-to-r from-brand-light to-brand bg-clip-text text-transparent"
						>L360</span
					>
				</h1>
				<span class="ml-2 text-[10px] uppercase tracking-widest text-slate-600 font-medium"
					>Panel</span
				>
			</div>
			...
			<div class="p-3 border-t border-white/[0.06] space-y-2">
				<div class="flex items-center gap-3 px-3 py-3 rounded-lg bg-white/[0.02]">
					<div
						class="w-8 h-8 rounded-full bg-brand/15 flex items-center justify-center text-brand text-xs font-bold shrink-0"
					>
						{{ userInitial }}
					</div>
					<div class="min-w-0">
		...
		<div class="flex-1 flex flex-col min-w-0 overflow-hidden">
			<!-- Topbar -->
			<header
				class="h-16 flex items-center justify-between px-4 lg:px-8 border-b border-white/[0.06] bg-[#060809]/80 backdrop-blur-xl shrink-0"
			>
				<button
